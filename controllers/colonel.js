...
module.exports.saveColonel = async (ctx, next) => {
  try {
    isLogin(ctx, next);
    const {
      name,
...
module.exports.getColonel = async (ctx, next) => {
  try {
    isLogin(ctx, next);
    const {
      pageIndex,
    ...
    res = await mysql("cPublicNumber")
      ...
      .where(function () {
        ...
        if (topCostS) {
          this.where("topCost", ">", topCostS);
        }
        if (topCostE) {
          this.where("topCost", "<", topCostE);
        }
        if (secondTitleS) {
          this.where("secondTitle", ">", secondTitleS);
        }
        if (secondTitleE) {
          this.where("secondTitle", "<", secondTitleE);
        }
        if (secondCostS) {
          this.where("secondCost", ">", secondCostS);
        }
        if (secondCostE) {
          this.where("secondCost", "<", secondCostE);
        }
        if (womenRatioS) {
...
module.exports.getColonelCount = async (ctx, next) => {
  try {
    isLogin(ctx, next);
    const {
      publicNumber,
...
module.exports.deleteColonel = async (ctx, next) => {
  try {
    isLogin(ctx, next);
...
module.exports.getOneColonel = async (ctx, next) => {
  try {
    isLogin(ctx, next);
    const { id } = ctx.request.query;
    res = await mysql("cPublicNumber").where({
...
module.exports.updateColonel = async (ctx, next) => {
  try {
    isLogin(ctx, next);
    const {
      id,
